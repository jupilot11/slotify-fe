...
function planFeatures(plan: SubscriptionPlan): string[] {
  const features = [
    ...
    plan.max_services != null
      ? `Up to ${plan.max_services} service${plan.max_services > 1 ? 's' : ''} per business`
  ...
  if (plan.trial_period_days != null && plan.trial_period_days > 0) {
    features.unshift(`${plan.trial_period_days}-day free trial`)
  }
  return features
}
...
function PricingCard({ plan, isPopular }: { plan: SubscriptionPlan; isPopular: boolean }) {
  return (
    <div
      className={`relative flex flex-col h-full rounded-2xl p-8 transition-all duration-200 ${isPopular
    ...
    >
      ...
      <Link href="/login" className="block">
        <button
          type="button"
          className={`w-full rounded-xl py-3 text-sm font-semibold transition-colors ${isPopular
...
export default function PricingSection({ plans }: { plans: SubscriptionPlan[] }) {
  return (
