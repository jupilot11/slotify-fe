...
export async function loginUser(payload: LoginRequest): Promise<LoginResponse> {
  ...
  // Restore the session returned by the edge function into the client-side Supabase instance.
  // The edge function uses the service role key to call signInWithPassword and returns the
  // resulting tokens; we must set them here so the browser client has an authenticated session.
